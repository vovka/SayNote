...
- `frontend/workflows/process-upload-job.ts`: Vercel/local workflow entrypoint for processing accepted jobs.
- `backend/worker/storage/r2.ts`: storage client is hardwired to Cloudflare R2 account-based endpoint.
- `backend/worker/db.ts`: processing steps require direct `DATABASE_URL` access.
...
- Processing is async: Next.js creates jobs, then a workflow claims the specific job from Postgres.
...
4. Workflow steps use `DATABASE_URL`, load encrypted provider credentials, download audio from R2, call Groq/OpenRouter, and write notes/categories back to Supabase.
...
- Optional local workflow tuning: `PROCESSING_RETRY_DELAY_MS`.
...
- Let the workflow run and confirm job transition to `completed` plus note creation.
...
- Workflow steps use direct Postgres access, so Supabase project credentials alone are insufficient without `DATABASE_URL`.
